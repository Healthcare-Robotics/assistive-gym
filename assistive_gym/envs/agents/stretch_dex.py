...
class StretchDex(Robot):
    def __init__(self, controllable_joints='right'):
        right_arm_joint_indices = [0, 1, 4, 6, 7, 8, 9, 10, 12, 13] # Controllable arm joints
        ...
        right_gripper_indices = [15, 17] # Gripper actuated joints
        left_gripper_indices = right_gripper_indices # Gripper actuated joints
        right_tool_joint = 19 # Joint that tools are attached to
        ...
        right_gripper_collision_indices = list(range(44)) # Used to disable collision between gripper and tools
        ...
        tool_orient_offset = {'scratch_itch': [0, 0, 0], # RPY orientation offset between tool and robot tool joint
                              ...
                              'comfort_taking_medicine': [0, np.pi/2.0, 0]}
    ...
    def init(self, directory, id, np_random, fixed_base=False):
        ...
        self.body = p.loadURDF(os.path.join(directory, 'stretch_dex', 'stretch_dex.urdf'), useFixedBase=False, basePosition=[-1, -1, 0.09], physicsClientId=id)
        super(StretchDex, self).init(self.body, id, np_random)
        ...
        # Recolor robot - copied from stretch.py, so might not be accurate
        white = [1, 1, 1, 1]
