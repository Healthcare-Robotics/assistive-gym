...
class Jaco(Robot):
    def __init__(self, arm='right'):
        ...
        gripper_pos = {'scratch_itch': [1]*3, 'feeding': [1.33]*3, 'drinking': [0.63]*3, 'bed_bathing': [1.1]*3} # Gripper open position for holding tools
        right_tool_joint = 8 # Joint that tools are attached to
        left_tool_joint = right_tool_joint # Joint that tools are attached to
        tool_pos_offset = {'scratch_itch': [0, 0, 0.02], 'feeding': [0.1, -0.0225, 0.03], 'drinking': [0.05, -0.005, 0], 'bed_bathing': [-0.01, 0, 0.03]} # Position offset between tool and robot tool joint
        tool_orient_offset = {'scratch_itch': [0, -np.pi/2.0, 0], 'feeding': [-0.1, -np.pi/2.0, 0], 'drinking': [0, -np.pi/2.0, np.pi/2.0], 'bed_bathing': [0, -np.pi/2.0, 0]} # RPY orientation offset between tool and robot tool joint
        ...
        pos = [-0.35, -0.3, 0.3]
        toc_base_pos_offset = {'scratch_itch': pos, 'feeding': pos, 'drinking': pos, 'bed_bathing': [-0.05, 1.05, 0.6]} # Robot base offset before TOC base pose optimization
        toc_ee_orient_rpy = {'scratch_itch': [0, np.pi/2.0, 0], 'feeding': [np.pi/2.0, 0, np.pi/2.0], 'drinking': [0, np.pi/2.0, 0], 'bed_bathing': [0, np.pi/2.0, 0]} # Initial end effector orientation
