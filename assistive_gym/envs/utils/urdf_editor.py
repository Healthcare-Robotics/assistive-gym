...
class UrdfVisual(object):
    ...
    def __init__(self):
        self.origin_rpy = [1, 2, 3]
        self.origin_xyz = [4, 5, 6]
        self.geom_type = p.GEOM_CAPSULE
        self.geom_radius = 1
        self.geom_extents = [7, 8, 9] # it's actually tuple
        self.geom_length = 10
    ...
    def writeVisualShape(self, file, urdfVisual, precision=5):
        ...
        if urdfVisual.geom_type == p.GEOM_MESH:
            str = '\t\t\t\t<mesh filename=\"{}\" scale=\"{:.{prec}f} {:.{prec}f} {:.{prec}f}\"/>\n'.format(
                urdfVisual.geom_meshfilename, urdfVisual.geom_meshscale[0], urdfVisual.geom_meshscale[1],
    ...
    def writeCollisionShape(self, file, urdfCollision, precision=5):
        ...
        if urdfCollision.geom_type == p.GEOM_MESH:
            str = '\t\t\t\t<mesh filename=\"{}\" scale=\"{:.{prec}f} {:.{prec}f} {:.{prec}f}\"/>\n'.format(
                ...
                urdfCollision.geom_meshscale[0], urdfCollision.geom_meshscale[1], urdfCollision.geom_meshscale[2],
                prec=precision)
            file.write(str)
